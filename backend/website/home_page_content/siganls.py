...
from .models import ClientsLogos, ClientsResponse, FewSuccessStories, ManagementTeam, MissionAndVision, PersonalImages
...
@receiver(pre_delete, sender=PersonalImages)
def before_delete_my_PersonalImages(sender, instance, **kwargs):
    try:
        id = get_file_id(instance.image_url)
        delete_file(id)
    except Exception as e:
...
@receiver(pre_delete, sender=FewSuccessStories)
def before_delete_my_FewSuccessStories(sender, instance, **kwargs):
    try:
        id = get_file_id(instance.image_url)
        delete_file(id)
    except Exception as e:
